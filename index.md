...
本系统采用 B/S 架构，并且将前后端进行分离，前端做成单页应用。后端做成 RESTful API，通过 AJAX 请求进行数据交互。但本文所研究的内容只限于前端单页应用的开发，故不涉及后端服务器部分。

整个前端项目使用了 React 框架开发，单纯使用 React 并不能开发一个功能完备的应用。所以我们必须加上一些必要的项目依赖。例如 react-router，它能够让你的单页应用像普通网站一样可以通过不同的 URL 跳转到不同的界面。还有 Redux，它能帮助我们管理所有要在多个组件中共享的数据，并优雅的使用他们。
